...
class LinkedInSignUp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isAuthorized: false,
  ...
  handlePostMessage = async event => {
    if (event.data.type === 'profile') {
      ...
      await createTalent({ body: body });

      let bodyLogin = {
        linkedInId: _.get(profile, 'id', ''),
      };

      const userInformations = await loginTalent({ body: bodyLogin });
...
export default withRouter(LinkedInSignUp);
